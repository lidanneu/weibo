...
BLOGGERS = [
...

    {
        ...
        "url": "https://weibo.com/u/7670330100",
...

    {
        ...
        "url": "https://weibo.com/u/8019050753",

        "dir": "weibo_8019050753",
...
def fetch_weibo_posts(uid, session):
    ...
    for page in range(1, 4):  # up to 3 pages (~30-45 posts)
        ...
        try:
            ...
            print(f"  [diag] page {page} -> HTTP {resp.status_code}, body_len={len(resp.text)}")
            ...
            try:
                ...
                print(f"  [JSON parse failed] page {page}: {je}")
                print(f"  [body500] {resp.text[:500]}")
                break

            if data.get("ok") != 1:

                print(f"  [API error] page {page}: ok={data.get('ok')!r} msg={data.get('msg', 'MISSING')!r}")
                print(f"  [body600] {resp.text[:600]}")
...
def main():
    ...
    if any_failed and not any_saved:
        print("  ALL FETCHES FAILED - cookie likely expired or API blocked. Exiting 1.")
        sys.exit(1)
